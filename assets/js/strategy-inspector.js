(function () {
    ...
    let activeName = Object.keys(strategies)[0] || '';
    ...
    function renderTabs() {
        document.getElementById('strategyTabs').innerHTML = Object.entries(strategies).map(([name, config]) => `<button type="button" class="strategy-tab${name === activeName ? ' active' : ''}" data-strategy="${esc(name)}"><strong>${esc(name)}</strong><span>${esc(config.desc || '')}</span></button>`).join('');
    ...
    function renderOverview(strategy) {
        ...
        document.getElementById('strategyOverview').innerHTML = `
            <div class="overview-main"><span class="overview-label">当前策略</span><strong class="overview-value">${esc(activeName)}</strong><span class="overview-detail">${esc(strategy.desc || '')}</span></div>
