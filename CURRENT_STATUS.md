...
- 当前线上与本地生产策略版本均为 `v4.2.32`；Pages `main=6800d0c`，应用构建 `2026-08-24-02`，资源版本 `20260824-02`。本次发布为首页增加不超过 1023px 的手机精简浏览版：保留大盘、本机自选、每日结论、关键依据和 90 日单 K 线，支持触摸回看；1024px 及以上桌面终端与独立策略查看器边界保持不变。同时修复冷启动无数据时后台补数隐藏错误面板的问题；没有改变数据契约、策略参数、仓位或 `B/S`。发布前 249 项完整回归、隔离部署包多尺寸 UI / 桌面状态 / 真实数据流 smoke 与线上桌面状态 smoke 均通过，Pages API 已确认对应提交构建完成。
