...
- “可能偏强 / 可能承压 / 方向分化”只是美股主题到 A 股行业的开盘前推测，必须显示“待 A 股确认”；不生成买卖、仓位或必涨必跌结论，也不进入板块趋势分、核心门禁、个股策略、`B/S` 或收益计算。
...
- 板块趋势是独立只读数据层，批量扫描东方财富 A 股行业与概念板块；不进入 `INDEX_CONFIG`、`state.rawData`、`state.liveQuotes`、`state.confirmedStatus`、指标、策略、仓位、`B/S`、收益计算或核心建仓门禁。
