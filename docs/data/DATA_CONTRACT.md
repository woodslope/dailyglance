...
- 左侧实时报价按当前可见标的组成一个批次；批次返回前不得逐行改写 DOM，同一批结果只允许统一提交一次。接口只返回部分标的时，已返回数据与未返回标的的 last-good 数据仍一次性展示，但刷新快照必须记录并显示本轮覆盖数，不能伪装成全量成功。
- 当前单个标的的历史补数只更新内存、缓存和右侧面板，不得单独推进整张左侧列表的刷新时间。自选股历史数据允许受控并发请求；策略状态计算必须等本轮历史批次结束后再以时间片分段执行，队列收敛后统一渲染并提交列表快照。
- 同一刷新通道只允许一个进行中批次；标签页或列表成员在请求期间变化时，旧批次不得回写当前列表。价格快通道与历史/策略慢通道分别提交，不互相冒充对方的完成状态。
...
- 左侧固定观察 8 个指数：上证指数、深证成指、沪深300、中证500、中证1000、创业板指、科创50、北证50。八项共用历史/实时数据契约；只有沪深300、中证500、中证1000的数据变化会使当前标的的核心宽基环境决策失效（个股仓位不受该环境限制，但决策对象仍带市场标签），其他五项仅刷新观察展示。
...
- IndexedDB 只负责持久化确认历史、自选和搜索缓存，不是历史数据真实性的来源。
- IndexedDB 不存在、打开抛错、被其他页面阻塞或 3 秒内未完成时，启动必须结束阻塞加载，并降级为当前页面内存模式。已从历史 API 获取且校验通过的数据可继续展示和计算，但刷新或关闭页面后不保证保留；界面需给出非阻塞提示。
- 会话中 IndexedDB 事务无法创建、写入失败或版本切换时，本页剩余周期继续使用内存模式，不循环重试存储；单次读取失败只按缓存未命中处理，由确认历史数据路径补齐。
- 持久化降级不得改变 `confirmedData` / `liveQuote` / `liveBar` 的准入与入库语义；实时数据仍不能因为缓存异常而写成确认历史。
- 只有 IndexedDB 异常属于可降级场景。如果必需的首屏状态或其他启动步骤抛出无法安全恢复的异常，应进入带“重新加载”的终止错误态，不能留在无限加载中。

## API 职责

### 外部环境推测条

- 外部环境推测条复用历史版隔夜主题映射，只读取 `SOXX / SMH / QQQ / MSFT / NVDA / AMD / TSLA / LI / BOTZ / SYM` 最近一轮公开行情，并压缩为“半导体与算力 / AI 与云计算 / 智能电动车 / 机器人与智能制造”四组行业方向提示。机器人组使用 `BOTZ / SYM / TSLA / NVDA`，其中 `SYM` 替代当前行情源不返回的 `ROBO`。
- 方向计算区分主方向与确认项：半导体使用 `SOXX / SMH`、AI 使用 `QQQ`、智能车使用 `TSLA`、机器人使用 `BOTZ` 作为主方向，主方向在组合变化中按确认项的两倍权重计算且方向必须一致；两项主题必须 `2/2` 同向，四项主题至少 `3/4` 同向，否则显示“方向分化”。
- 页面主时间按各标的 `quoteAt` 转换为 `America/New_York` 交易日，显示“美股 MM/DD 收盘”；北京时间接口获取时间只放在悬浮说明中。标的交易日不一致时必须显式显示“美股日期不一致”，不得用请求时间伪装成外盘交易日。
- “可能偏强 / 可能承压 / 方向分化”只是美股主题到 A 股行业的开盘前推测，必须显示“待 A 股确认”；不生成买卖、仓位或必涨必跌结论，也不进入板块趋势分、核心宽基环境、个股策略、`B/S` 或收益计算。
- 外部行情使用独立 `localStorage` 键 `dg_external_lead_strip_v1`，同页进行中请求复用、成功或失败均受至少 60 秒冷却；板块趋势页未激活、页面不可见或当前页没有焦点行情租约时不得发起请求。请求超时固定为 5 秒，失败时可显示明确标记的最近缓存。

### 板块趋势快照

- 板块趋势是独立只读数据层，批量扫描东方财富 A 股行业与概念板块；不进入 `INDEX_CONFIG`、`state.rawData`、`state.liveQuotes`、`state.confirmedStatus`、指标、策略、仓位、`B/S`、收益计算或核心宽基环境。
- 行业板块按接口返回的总数分页扫描，每页 100 条、最多 6 页；概念板块只取按当日涨幅排序的第一页 100 条，作为独立“概念热点”列表，不参与行业趋势中位数、趋势分和数量统计。行业分页失败时快照标记为“部分完成”，不得把未扫描部分当成弱势板块。字段包括今日、5日、10日、60日涨跌、成交额、换手率、主力净流入占比以及上涨/下跌/平盘家数；行业趋势计算完成后，只为排名前 6 个行业板块受控并发补充最多六只当前活跃个股。
- 行业状态固定为“上涨趋势 / 刚刚转强 / 单日异动”；概念热点只展示今日、5日、10日、代表股等紧凑信息。“上涨趋势”要求5日、10日、60日涨幅同时为正、上涨家数占比至少 `55%`，且趋势分至少 `65`；“单日异动”必须明确说明当日强势不等于已形成趋势。
- 只有排名前 6 的行业趋势卡片尝试显示静态维护的 ETF 参考；明确对应关系标记为“官方指数 / 参考 ETF”，较宽泛的上级行业关系标记为“相近参考（非直接对应）”，无法确认时显示“暂无稳定对应 ETF”，不根据名称模糊猜测，不增加板块扫描 API。
- 趋势分只用于板块排序，由多周期动量、相对全体板块中位数的强度、上涨覆盖和资金状态构成；不生成买入、卖出、建议仓位或必涨结论。每个板块必须展示可执行的趋势失效观察条件。
- 活跃个股只作只读观察，不自动选股、不跳转到个股、不写入自选、不调用个股历史数据或策略决策。
- 同页进行中请求必须复用，成功或失败请求都受至少 60 秒冷却；板块趋势页未激活、页面不可见或当前页没有焦点行情租约时不得发起请求。板块列表分页和成分请求超时固定为 8 秒，分页请求与成分请求均最多 3 路并发。
- 快照只写入独立 `localStorage` 键 `dg_sector_trend_snapshot_v2`；旧版板块快照会在清理缓存时移除。缓存板块和活跃个股必须明确标记为缓存，不得被解释为确认历史，也不得影响现有历史 API 熔断、实时 overlay 与刷新快照。
...
- 历史缺口、过期确认日期、无可信行情时间或股票价格体系不一致时，拒绝 overlay；左侧仍可显示 quote。
- 同日实时 overlay、缓存 liveBar 与盘后待确认数据绝不写回 `confirmedData` 或 IndexedDB；只有校验后的历史 K 线可替换确认数据。
- 自动和手动刷新使用同一 gate。刷新条应准确区分确认历史、盘中临时、缓存盘中、盘后待确认和 quote-only。
