...
成本压力、三段时间窗口和股票/指数/行业分层是第三类的研究工具，不进入每日仓位计算，不在前端生成“观察级/验证级”状态，也不阻塞第二类语义修正。

## 核心策略变更的标准流程

下述流程只用于上表第三类改动。新增或复核候选信号时，默认按五步推进：
...
1. 快速筛选：普通信号使用 `scripts/strategy-batch-screen.js`；改变交易行为的仓位或防守候选使用对应脚本的 `--screen`。后者固定保留全部市场指数与 `seed` 阶段股票（当前为 `8 + 16` 个标的），只判断是否有决策变化、方向是否明显回归。它只能输出 `continue_full`、`reject` 或 `insufficient_evidence`，不改代码、不接入策略。
...
核心策略研究采用“自动评估、人工准入”的长期闭环。自动脚本只能输出 `recommend_shadow`、`insufficient_evidence`、`reject` 或基线控制状态，不能自动修改生产 `STRATEGIES`、`SIGNAL_RULES`、仓位、`B/S` 或版本号。这套闭环不适用于展示翻译或已有决策契约的语义修正。
